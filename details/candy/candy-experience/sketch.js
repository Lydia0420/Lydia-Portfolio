...
let bgm, clickSound, gameOverSound, winSound;
let winPlayed = false;



function preload(){
  bgm = loadSound("assets/candy_bg.mp3");
  clickSound = loadSound("buttonClick.mp3");
  gameOverSound = loadSound("gameOver.mp3");
  winSound = loadSound("win.mp3");
  

  
  candyImgs[0] = loadImage("assets/candy_red.png");
  candyImgs[1] = loadImage("assets/candy_orange.png");
  candyImgs[2] = loadImage("assets/candy_blue.png");
  candyImgs[3] = loadImage("assets/candy_yellow.png");
  candyImgs[4] = loadImage("assets/candy_pink.png");
}
...
function setup() {
  createCanvas(600, 800);
    ...
    //Start button 
   startButton = new Sprite(width/2, height/2 + 100, 150, 60, 'k');  
  startButton.color = '#C51643';
...
function draw() {
    ...
    textAlign(CENTER);
    textSize(42);
    text("Candy Soda", width/2, 200);

    textSize(22);
    text("Drag the candies into the bottle!\nDrop all 5 before time runs out!", width/2, 260);
  ...
  //bubbles
    if (frameCount % Math.max(10, 100 - dropped * 5) === 0) { 
...
function startGame() {
  ...
  candies.push(new Candy(100, 750, candyImgs[0]));
  candies.push(new Candy(180, 750, candyImgs[1]));
  candies.push(new Candy(260, 750, candyImgs[2]));
  candies.push(new Candy(340, 750, candyImgs[3]));
  candies.push(new Candy(420, 750, candyImgs[4]));
}
...
class Candy {
  ...
  if (this.sprite.y > bottleCapY && this.falling && !this.collected) {

    this.collected = true;        
    this.sprite.visible = false;  

    for (let i = 0; i < 15; i++) {
      spatters.push(new Spatter(this.sprite.x + 40, bottleCapY));
    }
...
function drawBottle() {
  ...
  bezierVertex(160, 240, 175, 245, 200, 225); // The first wave
  bezierVertex(208, 220, 220, 220, 230, 225); 
  bezierVertex(260, 240, 270, 240, 285, 225); 
  bezierVertex(300, 210, 315, 230, 330, 230); 
  bezierVertex(380, 210, 360, 250, 402, 225); // Five Waves
  
  bezierVertex(410, 245, 415, 290, 378, 328); 
  bezierVertex(378, 330, 300, 330, 175, 330);  //straight line
 ...
 beginShape();
  ...
  bezierVertex(399, 210, 375, 175, 381, 184); 
   bezierVertex(380, 180, 360, 167, 330, 190); //Waves above
 ...
 beginShape();
  vertex(380, 180); // Start point
   bezierVertex(380, 180, 360, 167, 330, 189); //bottom waves
   bezierVertex(310, 200, 285, 190, 270, 185); 
   bezierVertex(245, 175, 240, 185, 230, 190);
   bezierVertex(218, 200, 205, 200, 198, 195);
   bezierVertex(170, 170, 168, 195, 160, 188);
    bezierVertex(170, 180, 180, 150, 227, 130); //left 
    bezierVertex(227, 130, 300, 130, 334, 132); //top
  endShape();
}
...
function drawLabel() {
  ...
  ellipse(275, 405, 90, 90);
    // eyes
  fill(0); 
  ellipse(260, 390, 10, 10); // left eye
  ellipse(290, 390, 10, 10); // right eye
   // mouth
  noFill(); 
  stroke(0); 
  strokeWeight(4); 
